...
async function processJob(job: any) {
  ...
  if (job.payload && job.payload.url) {
    ...
    for (let p = 25; p <= 100; p += 25) {
      ...
      try { getIO().emit('jobProgress', { id: job.id, progress: p }); } catch(e) {}
      if (p < 100) await sleep(500); // 1.5 seconds total delay for animation
    }
...
async function sweepZombieJobs() {
  ...
  const zombieJobs = await prisma.$queryRaw<any[]>`
    UPDATE "Job"
...
export async function startWorker() {
  ...
  while (!isShuttingDown) {
    try {
      ...
      const rateLimitedQueueIds: string[] = [];
      const queuesWithLimits = await prisma.queue.findMany({
      ...
      const excludeSql = rateLimitedQueueIds.length > 0
        ? Prisma.sql`AND j."queueId" NOT IN (${Prisma.join(rateLimitedQueueIds)})`
        : Prisma.sql``;
      ...
      await Promise.all(claimedJobs.map(async (job) => {
        ...
        try {
          getIO().emit('jobUpdated', { id: job.id, status: 'running' });
        } catch (e) {}
        ...
        if (finalStatus === 'completed' || finalStatus === 'dead_letter') {
          ...
          if (finalStatus === 'completed') {
            ...
            for (const dep of dependents) {
              ...
              if (allCompleted) {
                await prisma.job.update({ where: { id: dep.id }, data: { status: 'queued' } });
                try { getIO().emit('jobUpdated', { id: dep.id, status: 'queued' }); } catch (e) {}
              }
            }
          } else if (finalStatus === 'dead_letter') {
            // Cascade failure to downstream dependents recursively
            const cascadeFail = async (parentJobId: string) => {
              const dependents = await prisma.job.findMany({
                where: { status: 'waiting', dependencies: { some: { id: parentJobId } } }
              });
              for (const dep of dependents) {
                await prisma.job.update({ where: { id: dep.id }, data: { status: 'dead_letter' } });
                ...
                try { getIO().emit('jobUpdated', { id: dep.id, status: 'dead_letter' }); } catch (e) {}
        ...
        try {
          getIO().emit('jobUpdated', { id: job.id, status: finalStatus === 'queued' ? 'failed_retrying' : finalStatus });
        } catch (e) {}
